...
def serialize_private(key: dict, path: str) ->None:
    try:
        with open(path, "wb") as file:
            file.write(key['private'].private_bytes(encoding=serialization.Encoding.PEM,
...
def serialize_public(key: dict, path: str) ->None:
    try:
        with open(path, "wb") as file:
            file.write(key['public'].public_bytes(encoding=serialization.Encoding.PEM,
             format=serialization.PublicFormat.SubjectPublicKeyInfo))
    except Exception as e:
        print("Возникла ошибка при сиреализации приватного ключа: ", e)

def encrypt_sym_key(asym_key: dict, sym_key: bytes) -> bytes:
    key = asym_key['public'].encrypt(sym_key,padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()),algorithm=hashes.SHA256(),label=None))
    return key

def decrypt_sym_key(asym_key: dict, key: bytes) -> bytes:
        de_key = asym_key['private'].decrypt(key,padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()),algorithm=hashes.SHA256(),label=None))
        return de_key
